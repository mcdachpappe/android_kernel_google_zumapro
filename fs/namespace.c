...
#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
/* A copy of alloc_vfsmnt() that allocates the sus mnt_id to mnt */
					...
					int flag)
{
	...
	if (susfs_is_current_zygote_domain()) {
		/* Check if mnt_devname is KSU
		 * and assign sus mnt_id */
		if (!strcmp(old->mnt_devname, "KSU")) {
			mnt = susfs_alloc_sus_vfsmnt(old->mnt_devname);
			goto bypass_orig_flow;
		}
		/* Retrieve source mount path */
		...
		/* If mnt_group_id is 1 and mountpoint
		 * is not "/", assign sus mnt_id */
		if (old->mnt_group_id == 1 && strcmp(path_ptr, "/")) {
			mnt = susfs_alloc_sus_vfsmnt(old->mnt_devname);
			kfree(src_path);
			goto bypass_orig_flow;
		}
		/* If mount path begins with
		 * "/adb/" assign sus mnt_id */
